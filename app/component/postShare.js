...
export default class PostShare extends Component {
        ...
        AsyncStorage.getItem("user")
            .then(data=> {
                if (data) {
                    ...
                    console.log(`${WEBSITEURL}/getPostCircle.php?uID=${this.state.user.ID}`);
                    fetch(`${WEBSITEURL}/getPostCircle.php?uID=${this.state.user.ID}`)
                        .then((response)=>response.json())
                        .then((responseJson)=>{
                            this.setState({items:this.state.items.concat(responseJson)});
    ...
    static navigationOptions =({ navigation, navigationOptions }) =>{
        return {
            headerTitle: (<LogoTitle title="发布分享"/>),
            headerLeft: (
    ...
    submitText(){
        console.log(`${WEBSITEURL}/postShare.php?uID=${this.state.user.ID}`);
        console.log(this.state.user)
        fetch(`${WEBSITEURL}/postShare.php?uID=${this.state.user.ID}&cID=0`,{
            method:"post",
            headers: {
        ...
        return (
            <View style={[styles.container]}>
                <ImageBackground source={require("../../www/img/content_bg.jpg")} style={styles.containerImage}>
                    <TextInput value={this.state.title} onChangeText={(text)=>this.setState({title:text})} placeholder="分享标题" underlineColorAndroid="transparent" style={{ marginRight:20,marginLeft:20,marginTop:30,marginBottom:10, borderColor:"black",borderWidth:1,borderRadius:5}}/>
                    <TextInput value={this.state.content} onChangeText={(text)=>this.setState({content:text})} multiline = {true} placeholder="分享内容" style={{marginRight:20,marginLeft:20,marginTop:10,marginBottom:10,textAlignVertical: 'top',borderColor:"black",borderWidth:1,height:300,borderRadius:5,}}/>

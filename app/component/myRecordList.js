...
export default class MyRecordList extends Component {
    state = {
        data: new Array(),
        loadingVisible:true,
        loadedState:false,
    ...
    componentDidMount=function(){
        console.log(`${WEBSITEURL}/userRecordList.php?id=0&uID=${JSON.stringify(this.props.navigation.getParam('uID',"1"))}`);
        fetch(`${WEBSITEURL}/userRecordList.php?id=0&uID=${JSON.stringify(this.props.navigation.getParam('uID',"1"))}`)
    ...
    static navigationOptions =({ navigation, navigationOptions }) =>{
        return {
            headerTitle: (<LogoTitle title="我的录音"/>),
            headerLeft: (
                ...
                <RefreshableFlatList
                    ...
                    onLoadMore={() => new Promise((r) => {
                        setTimeout(() => {
                            ...
                            console.log(`${WEBSITEURL}/userRecordList.php?id=${no}&uID=${JSON.stringify(this.props.navigation.getParam('uID',"1"))}`);
                            fetch(`${WEBSITEURL}/userRecordList.php?id=${no}&uID=${JSON.stringify(this.props.navigation.getParam('uID',"1"))}`)
                                .then((response)=>response.json())
                                .then((responseJson)=>{

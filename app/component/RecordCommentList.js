...
export default class RecordCommentList extends Component {
    state = {
        data: new Array(),
        loadingVisible:true,
        loadedState:false,
    ...
    componentDidMount=function(){
        console.log(`${WEBSITEURL}/RecordCommentList.php?id=0&rID=${this.props.navigation.getParam("rID")}`)
        fetch(`${WEBSITEURL}/RecordCommentList.php?id=0&rID=${this.props.navigation.getParam("rID")}`)
                ...
                <RefreshableFlatList
                    ...
                    onRefreshing={() => new Promise((r) => {
                        setTimeout(() => {
                            fetch(`${WEBSITEURL}/RecordCommentList.php?id=0&rID=${this.props.navigation.getParam("rID")}`)
                                .then((response)=>response.json())
                                .then((responseJson)=>{
                                    this.setthis.setState({data:responseJson});
                    ...
                    onLoadMore={() => new Promise((r) => {
                        setTimeout(() => {
                            ...
                            fetch(`${WEBSITEURL}/RecordCommentList.php?id=${no}&rID=${this.props.navigation.getParam("rID")}`)
                                .then((response)=>response.json())
                                .then((responseJson)=>{

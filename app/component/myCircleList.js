...
export default class MyCircleList extends Component {
    state = {
        data: new Array(),
        loadingVisible:true,
        loadedState:false,
    ...
    componentDidMount=function(){
        console.log(`${WEBSITEURL}/userCircleList.php?id=0&uID=${JSON.stringify(this.props.navigation.getParam('uID',"1"))}`);
        fetch(`${WEBSITEURL}/userCircleList.php?id=0&uID=${JSON.stringify(this.props.navigation.getParam('uID',"1"))}`)
    ...
    static navigationOptions =({ navigation, navigationOptions }) =>{
        return {
            headerTitle: (<LogoTitle title="我的学圈"/>),
            headerLeft: (
                ...
                <RefreshableFlatList
                    ...
                    onLoadMore={() => new Promise((r) => {
                        setTimeout(() => {
                            ...
                            console.log(`${WEBSITEURL}/userCircleList.php?id=${no}&uID=${JSON.stringify(this.props.navigation.getParam('uID',"1"))}`);
                            fetch(`${WEBSITEURL}/userCircleList.php?id=${no}&uID=${JSON.stringify(this.props.navigation.getParam('uID',"1"))}`)
                                .then((response)=>response.json())
                                .then((responseJson)=>{

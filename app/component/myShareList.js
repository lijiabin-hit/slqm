...
export default class MyShareList extends Component {
    state = {
        data: new Array(),
        loadingVisible:true,
        loadedState:false,
    ...
    componentDidMount=function(){
        console.log(`${WEBSITEURL}/userShareList.php?id=0&uID=${JSON.stringify(this.props.navigation.getParam('uID',"1"))}`);
        fetch(`${WEBSITEURL}/userShareList.php?id=0&uID=${JSON.stringify(this.props.navigation.getParam('uID',"1"))}`)
    ...
    static navigationOptions =({ navigation, navigationOptions }) =>{
        return {
            headerTitle: (<LogoTitle title="我的分享"/>),
            headerLeft: (
            ...
            headerRight: (
                ...
                <View style={{flexDirection:"row"}}>
                    <TouchableOpacity onPress={()=>navigation.navigate('PostShare')}><Text><Icon name="md-add" size={23} color="white"/>&nbsp;&nbsp;&nbsp;&nbsp;</Text></TouchableOpacity>
                </View>
            ),
        }
    }
                ...
                <RefreshableFlatList
                    ...
                    onLoadMore={() => new Promise((r) => {
                        setTimeout(() => {
                            ...
                            console.log(`${WEBSITEURL}/userShareList.php?id=${no}&uID=${JSON.stringify(this.props.navigation.getParam('uID',"1"))}`);
                            fetch(`${WEBSITEURL}/userShareList.php?id=${no}&uID=${JSON.stringify(this.props.navigation.getParam('uID',"1"))}`)
                                .then((response)=>response.json())
                                .then((responseJson)=>{

...
export default class MyCommentList extends Component {
    ...
    componentDidMount=function(){
        console.log(`${WEBSITEURL}/UserCommentList.php?id=0&uID=${this.props.navigation.getParam("uID")}`)
        fetch(`${WEBSITEURL}/UserCommentList.php?id=0&uID=${this.props.navigation.getParam("uID")}`)
    ...
    static navigationOptions =({ navigation, navigationOptions }) =>{
        return {
            headerTitle: (<LogoTitle title="我的评论"/>),
            headerLeft: (
        ...
        return (
            <View style={styles.container}>
                <RefreshableFlatList
                    ...
                    renderItem={({ item }) => (
                        <TouchableOpacity onPress={()=>{item.share==0?this.props.navigation.navigate("RecordDetail",{rID:item.record,title:item.title}):this.props.navigation.navigate("ShareDetail",{sID:item.share,title:item.title})}} key={item.id} style={styles.Item}>
                    ...
                    onLoadMore={() => new Promise((r) => {
                        setTimeout(() => {
                            ...
                            fetch(`${WEBSITEURL}/UserCommentList.php?id=${no}&uID=${this.props.navigation.getParam("uID")}`)
                                .then((response)=>response.json())
                                .then((responseJson)=>{

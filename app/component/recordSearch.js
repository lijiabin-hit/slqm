...
class SearchList extends Component{
    ...
    componentDidMount=function(){
        console.log(`${WEBSITEURL}/SearchRecordList.php?id=0&content=${this.props.content}`);
        fetch(`${WEBSITEURL}/SearchRecordList.php?id=0&content=${this.props.content}`)
        ...
        return (
            <View style={styles.container}>
                <RefreshableFlatList
                    ...
                    renderItem={({ item }) => (
                        <TouchableOpacity onPress={()=>this.props.that.props.navigation.navigate("RecordDetail",{rID:item.id,title:item.title})} key={item.id} style={styles.Item}>
                    ...
                    onLoadMore={() => new Promise((r) => {
                        setTimeout(() => {
                            ...
                            console.log(`${WEBSITEURL}/SearchRecordList.php?id=${no}&content=${this.props.content}`);
                            fetch(`${WEBSITEURL}/SearchRecordList.php?id=${no}&content=${this.props.content}`)
                                .then((response)=>response.json())
                                .then((responseJson)=>{
